...
public class AiChatbotHttpFacade {
    ...
    private AiChatbotGroupPolicy policy(AiChatbotGroupPolicySaveRequest body, String connectionId, String channelId) { return new AiChatbotGroupPolicy(connectionId, channelId, body.enabled(), body.randomProbability(), body.groupContextLimit(), body.personalContextLimit(), body.contextExpansionLimit(), body.cooldownSeconds(), body.hourlyReplyLimit(), body.quietHoursStart(), body.quietHoursEnd(), body.systemPrompt(), body.persona(), body.randomToolCallingEnabled(), body.longTermMemoryEnabled(), body.semanticMemoryTopK(), body.agentCode(), body.providerCode(), body.modelCode(), body.mentionReplyInjection()); }
