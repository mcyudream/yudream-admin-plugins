...
import { FaButton, FaCard, FaFileUpload, FaIcon, FaInput, FaNumberField, FaPageHeader, FaPageMain, FaResponsiveTable, FaSelect, FaSwitch, useFaModal, useFaToast } from '@yudream/components'
...
function confirmDeleteMap() { modal.confirm({ title: '删除地图', content: '确认删除此地图 ZIP 附件吗？', onConfirm: () => props.model.deleteMap() }) }
</script>
<template>
  ...
  <FaPageMain><form class="grid gap-4" @submit.prevent="model.saveServer">
    ...
    <div v-if="model.serverForm.id" class="mt-4 grid gap-3 rounded-lg border p-4"><div class="flex flex-wrap items-center justify-between gap-2"><div><h3 class="text-base font-semibold">地图 ZIP</h3><p class="text-sm text-muted-foreground">上传后默认私有；公开地图可由普通用户下载。</p></div><FaFileUpload v-model="mapUploadFiles" :max="1" :disabled="model.mapOperating" :before-upload="beforeUploadMap" :http-request="uploadMapRequest" @on-success="clearUploadedMapFiles"><FaButton as="span" type="button" variant="outline" :loading="model.mapOperating"><FaIcon name="i-ri:upload-2-line" />上传 ZIP</FaButton></FaFileUpload></div><template v-if="model.selectedServer?.map"><div class="mc-map-admin-row"><span>{{ model.selectedServer.map.originalName || '地图 ZIP' }}</span><label class="inline-flex items-center gap-2 text-sm">公开下载 <FaSwitch :model-value="model.selectedServer.map.publicAccess" :disabled="model.mapOperating" @update:model-value="model.setMapPublicAccess(Boolean($event))" /></label><div class="flex gap-2"><FaButton size="sm" type="button" variant="outline" :loading="model.mapOperating" @click="model.downloadMap(true)"><FaIcon name="i-ri:download-line" />下载</FaButton><FaButton size="sm" type="button" variant="destructive" :disabled="model.mapOperating" @click="confirmDeleteMap"><FaIcon name="i-ri:delete-bin-line" />删除</FaButton></div></div></template><p v-else class="text-sm text-muted-foreground">尚未上传地图 ZIP。</p></div>
