  ...
  <FaPageMain>
    <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
      <form class="grid max-w-4xl gap-4" @submit.prevent="save">
        ...
        <section class="grid gap-3 rounded border p-4"><h3>工具调用与长期记忆</h3><div class="flex flex-wrap gap-4"><FaSwitch v-model="form.randomToolCallingEnabled">随机回复时允许调用工具</FaSwitch><FaSwitch v-model="form.longTermMemoryEnabled">启用长期向量记忆</FaSwitch><FaSwitch v-model="form.groupToolOpenAccess">群内开放只读工具（不要求绑定账号权限）</FaSwitch></div><label class="grid max-w-xs gap-2">记忆召回条数<FaNumberField v-model="form.semanticMemoryTopK" :min="1" :max="20" /></label></section>
