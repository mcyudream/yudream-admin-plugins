...
const columns = ref<TableColumn<UpdateReleaseRecord>[]>([
  ...
  { id: 'changes', header: '变更', width: 90 },
  { id: 'artifacts', header: '制品', width: 80 },
  { id: 'status', header: '状态', width: 120 },
  { id: 'operations', header: '操作', width: 280 },
])
...
async function submitRelease() {
  ...
  try {
    await api.saveRelease({
      ...
      publishedAt: releaseForm.publishedAt || undefined,
      forceUpdate: releaseForm.forceUpdate,
      enabled: releaseForm.enabled,
      externalUrl: releaseForm.externalUrl.trim() || undefined,
    })
    toast.success('发布已保存')
...
async function onArtifactFileChange(event: Event) {
  const input = event.target as HTMLInputElement
  const file = input.files?.[0]
  if (!file) {
    return
  }
  if (!artifactForm.filename) {
    artifactForm.filename = file.name
  }
  try {
    saving.value = true
    const data = await readFileAsBase64(file)
    await api.saveFileArtifact(detailVersion.value, {
      mode: 'file',
      ...
      targetPlatforms: artifactForm.targetPlatforms
        ...
        .map(item => item.trim())
        .filter(Boolean),
      filename: artifactForm.filename || file.name,
...
<template>
	<section class="ymcl-releases-page">
		<FaPageHeader
			title="更新发包"
			description="YMCL 自有更新平台：维护版本发布与制品（外部 URL 或站内文件）。启动器通过 env 指向本平台拉取 latest/manifest。"
			class="mb-0"
		>
		...
		<FaPageMain>
			<FaCard content-class="ymcl-card-content">
				<div class="ymcl-toolbar">
					<p class="ymcl-muted">
						...
						· 启动器 env：<code>VITE_YMCL_UPDATE_URL</code>
						或 <code>VITE_YMCL_CONTENT_URL</code>
					</p>
					<FaButton type="primary" @click="openCreateRelease">
						<FaIcon name="i-ri:add-line" />
						新建发布
			...
			>
				<form class="ymcl-form" @submit.prevent>
					...
					<div class="ymcl-form-row">
						<label class="ymcl-label">完整更新日志链接（可空；启动器「查看完整更新日志」将打开此地址）</label>
						<FaInput v-model="releaseForm.externalUrl" placeholder="https://... 或 https://yda/changelog" clearable />
					</div>
					<div class="ymcl-form-row">
						<label class="ymcl-label">发布时间（ISO 或毫秒时间戳，可空）</label>
						<FaInput v-model="releaseForm.publishedAt" placeholder="2026-03-20T00:00:00Z" clearable />
					</div>
					<div class="ymcl-form-row">
		...
		>
			<form class="ymcl-form" @submit.prevent>
				...
				<div class="ymcl-form-row">
					<label class="ymcl-label">文件名（必填）</label>
				...
				<div class="ymcl-form-row">
					...
					<FaTextarea v-model="artifactForm.signature" :rows="4" />
				</div>
				<div v-if="artifactForm.mode === 'file'" class="ymcl-form-row">
					<label class="ymcl-label">选择文件上传到站内</label>
					<input type="file" :disabled="saving" @change="onArtifactFileChange">
				</div>
			</form>
			<template #footer>
