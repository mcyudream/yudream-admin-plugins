...
const columns: TableColumn<CategoryView>[] = [
  ...
  { id: 'materials', header: '物料数', width: 100 },
...
function confirmDelete(row: CategoryView) {
  confirm.confirm({
    title: '删除分类',
    content: `确认删除分类「${row.name}」吗？该分类下有 ${row.materials} 个物料时必须先把它们移出。`,
    onConfirm: () => model.removeCategory(row),
  })
}

onMounted(() => model.loadCategories())
</script>

<template>
  <FaPageHeader title="物料分类" description="分类对所有用户共享，删除前需移出其中物料">
  ...
  <FaPageMain>
    <FaResponsiveTable
      ...
      empty-text="还没有分类"
    >
      <template #cell-materials="{ row }">{{ row.original.materials }}</template>
      ...
      <template #card="{ row }">
        <div class="flex flex-col gap-2 text-sm">
          <div class="flex items-center justify-between">
            ...
            <span class="text-secondary-foreground/60">{{ row.materials }} 个物料</span>
          </div>
    ...
    <FaModal v-model="editOpen" :title="editTarget ? '编辑分类' : '新建分类'" :confirm-button-loading="saving" @confirm="submit">
      <div class="flex flex-col gap-3">
        <label class="flex flex-col gap-1 text-sm">
          ...
          <FaInput v-model="editName" maxlength="30" placeholder="如：宣传物料" />
        </label>
        <label class="flex flex-col gap-1 text-sm">
