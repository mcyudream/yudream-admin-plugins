 ...
 * 与 YMCL-Axolotl App.vue 的 ymclNavIconMap 语义对齐。
 */
...
/** Remix 名反查启动器内置 token。 */
export function nativeTokenFromRemix(raw?: string | null): string | undefined {
  const value = raw?.trim()
  if (!value) return undefined
  if (Object.prototype.hasOwnProperty.call(NATIVE_ICON_TOKENS, value)) {
    return value
  }
  for (const [token, meta] of Object.entries(NATIVE_ICON_TOKENS)) {
    if (meta.remix === value) return token
  }
  return undefined
}

/**
 * 写入/展示前的规范值：能映射到启动器内置 token 的一律用 token，
 * 保证下发给 YMCL 的是内置图标，而不是 Remix 字符串。
 */
export function preferredIconValue(raw?: string | null): string {
  const value = raw?.trim()
  if (!value) return ''
  return nativeTokenFromRemix(value) || value
}

export function isNativeIconValue(raw?: string | null): boolean {
  return Boolean(nativeTokenFromRemix(raw))
}

export function findCatalogIcon(name?: string | null): IconCatalogItem | undefined {
  const value = preferredIconValue(name)
  if (!value) return undefined
  return ICON_CATALOG.find(item => (item.token || item.name) === value || item.name === remixIconName(value))
}
...
export function iconCatalogDeduped(): IconCatalogItem[] {
  ...
  for (const item of ICON_CATALOG) {
    const key = item.token || item.name
