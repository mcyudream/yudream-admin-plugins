...
- A soft dependency in `softdepend` controls load order only when the provider is present and enabled. Runtime restores enabled soft providers before creating the consumer class loader, so their API classes are visible; missing, disabled, or failed providers do not block the consumer. Do not promote an integration to `depend` merely because its code imports the provider API: first decide whether the product can operate with that capability disabled. When it can, keep `softdepend`, test provider API availability before resolving or calling it, and return an explicit unavailable result or disable only the affected route, operation, menu section, scheduled work, and frontend controls. Isolate optional integration code so an unavailable provider cannot cause `NoClassDefFoundError` on an otherwise unrelated endpoint. Use `depend` only when the plugin's core business capability cannot operate without the provider. The `depend`/`softdepend` graph must be acyclic. A platform adapter must not softdepend a business plugin that already softdepends the adapter (`launcher-adapter` must not list `minecraft-server`; server-list pages and YMCL scopes are contributed by `minecraft-server` via `LauncherProvider`).
...
- Build route pages under `src/pages`, reusable UI under `src/components`, stateful workflows under `src/composables`, SDK-backed calls under `src/api`, and shared view models in `src/types.ts`. Export each `@PluginRoute.component` as that page from `src/index.ts`; do not share one shell across routes and guess the page from path or `meta.plugin`.
